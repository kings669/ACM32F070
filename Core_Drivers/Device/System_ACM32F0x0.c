...
uint32_t gu32_Clocksrc = CLK_SRC_RC64M;    
...
bool System_Clock_Init(uint32_t fu32_Clock)
{
    ...
    if (lu32_system_clk_source == CLK_SRC_XTH_PLL)   
    {
        ...
        if (0 == lu32_result)
        {
            ...
            SCU->RCHCR |= SCU_RCHCR_RCH_DIV;   // Divide RCH 16   
        }
        else
        {
...
/*********************************************************************************
* Function    : System_LVD_Config
* Description : set LVD RESET CONFIG
* Input       : LVD_InitTypeDef
* Output      : none
**********************************************************************************/
...
{   
    ...
    SCU->RSR |= BIT16;//clear reset reason
    
    temp=(LVD_Init->LVD_FLT_TIME<<9)|(LVD_Init->LVD_FLTEN<<8)|(LVD_Init->LVD_TRIM<<1)|(LVD_Init->LVDEN);
    SCU->LVDCFG = temp;
    System_Delay(100); 
    
    if(LVD_Init->LVDRST_EN==FUNC_ENABLE)
    {
       SCU->RCR |= BIT0;//LVD RESET  
    }
    else
    {
       SCU->RCR &=~ BIT0;//LVD RESET  
    ...
    if(LVD_Init->LVD_FLTCLK_SEL==FLTCLK_SEL_RC32k)
    {
       SCU->CCR2 |= BIT15;//RC32k  
    }
    else
    {
       SCU->CCR2 &=~ BIT15;//pclk/32
    }
}

/*********************************************************************************
* Function    : System_Core_Config
* Description : Initate some chip specific parameters before __main.  
* Input       : none  
* Output      : none   
* Author      : fw                         
**********************************************************************************/
